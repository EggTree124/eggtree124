# Hi 👋 I'm Po

🎮 Game Development Student from Indonesia
💻 Passionate about Game Programming, Graphics, and Engine Development

## 🚀 Currently Learning

* Godot Engine
* Modern C++
* OpenGL & Vulkan
* Blender
* Game Boy Advance Development (Butano)

## 🛠 Tech Stack

**Languages**
C • C++ • GDScript • Python

**Tools**
Godot • Blender • Git • GitHub • Linux • VS Code

## 📌 Featured Projects

* 🎮 **Hungry Ball** – A Game Boy Advance game built with Butano.
* ⚔️ **3D Character Controller** – A reusable controller with animation blending and combo attacks.
* 🧪 **Engine Experiments** – Rendering, ECS, physics, and optimisation projects.

## 📊 GitHub Stats
![GitHub Stats](https://github-readme-stats-sigma-five.vercel.app/api?username=EggTree124&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=EggTree124&layout=compact&theme=tokyonight)

## 🎯 Goals

* Build polished portfolio games
* Contribute to open source
* Learn graphics programming
* Become a professional game developer

## 🌐 Links

* 🎮 itch.io: https://eggtree124.itch.io
* 💼 GitHub: https://github.com/EggTree124

---

> *"Building games one commit at a time."*
